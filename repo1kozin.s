...
***************************************************************
** MAINルーチン
** ・LED初期化、ユーザスタック設定、タイマリセット
** ・問題文の表示、1文字ずつのタイピング入力受付
** ・制限時間内に正しく入力できた文字数やスコアを更新
**
** 使用レジスタ
**   D0.l : システムコール番号, GETSTRING戻り値(sz), タイマ値など汎用
**   D1.l : 問題の文字数, GETSTRINGの ch 引数
**   D2.l : システムコール用ポインタ引数 p（RETURN_Q/RETURN_IN/BUF/TT 等）
**   D3.l : 文字列長 size（PUTSTRING用）など一時レジスタ
**   D4.l : 正しく入力できた文字数カウンタ
**   D5.b : 入力1文字（BUFの中身）
**   A0.l : 現在の問題文字列へのポインタ（QUEST1 内で進行）
**   A1.l : 問題サイズ配列 quest_size へのポインタ
**   A2.l : 問題先頭アドレスの退避用ポインタ
**   SP   : USR_STK_TOP で初期化されるユーザスタック
**   SR   : ユーザモード／走行レベル0 に設定
**
** 破壊レジスタ：D0–D5, A0–A2
***************************************************************
.section .text

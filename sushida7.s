...
**sushida関係の初期化
	...
	lea.l	QUEST1, %a0		|a0 = QUEST1のアドレス動的|
	move.l	%a0, %a2		|a2 = QUEST1のアドレス保存|
	lea.l	quest_size, %a1		|a1 = 問題1のサイズのアドレス|
...
LOOP0:
	...
	addi.w	#1, correcttype			|correcttype++|
	subi.w	#1, misstype			|misstype--|
...
FINISH:
	...
	lea.l	correcttype, %a6
	jsr	DISPLAY_D_ASKII			|correcttypeの表示|
	...
	lea.l	misstype, %a6
	jsr	DISPLAY_D_ASKII			|misstypeの表示|
	...
	**打/秒の計算↓↓
	move.w	correcttype, %d0		|correcttypeは正解打数|
	mulu	#10, %d0			|割る数が0.1sec単位なので正解打数を10倍|
	move.w	all_time, %d1			|all_time終了までの経過時間|
	andi.l	#0x0000ffff, %d1		|意味ないと思うけど念の為マスク|
	divu	%d1, %d0			|正解打数÷経過時間|
	move.l	%d0, %d2			|結果のコピー|
	addi.l	#0x30, %d2
	move.b	%d2, FLOAT_DISPLAY1		|.w商のみを格納→1の位で1桁|

	LSR.l	#8, %d0				|4桁シフトして余りのみにする|
	LSR.l	#8, %d0				
	mulu	#10, %d0			|余りを10倍(筆算で言うと0おろし)|
...
**問題を追加する場合は以下にコピペで追加し，
**全問題数ALL_Qと配列quest_sizeを更新すること
**1問あたり13文字まで
QUEST1:
	.ascii "banana"
	.ds.b	16-6		
